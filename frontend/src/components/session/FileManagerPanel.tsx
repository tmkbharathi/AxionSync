...
export const FileManagerPanel = memo(({ 
  ...
  return (
    <div className={`h-full bg-slate-900/20 flex flex-col overflow-hidden transition-all duration-300 ease-in-out ${activeTab === 'files' ? 'block' : 'hidden md:flex'} ${isFilePanelCollapsed ? 'w-0 opacity-0 pointer-events-none' : 'w-full md:w-2/5 lg:w-1/3 opacity-100'}`}>
      {isFilesHidden ? (
        <div className="flex-1 flex flex-col items-center justify-center p-8 text-center bg-slate-900/30 border border-slate-800/60 rounded-xl m-4">
          <Lock className="w-10 h-10 mb-3 text-amber-400/80 animate-pulse" />
          <h3 className="text-base font-bold text-slate-300">File Library Hidden</h3>
          <p className="text-xs text-slate-500 max-w-xs mt-1">
            The room administrator has restricted file library access for this guest passcode.
          </p>
        </div>
      ) : (
        <>
